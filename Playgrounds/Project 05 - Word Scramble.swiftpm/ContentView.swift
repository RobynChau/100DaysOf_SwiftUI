...
struct ContentView: View {
    ...
    func addNewWord() {
        ...
        guard isLong(word: answer) else {
            wordError(title: "Word is short", message: "Word must be longer than 3 letters")
            return
        }
        guard isCopied(word: answer) else {
            wordError(title: "Word is copied", message: "Please don't copy the start word!")
            return
        }
    ...
    func isCopied(word: String) -> Bool {
        !(word==rootWord)
    }
